...
- Unsafe harness `runAttempt` diagnostic mode is implemented behind `plugins.entries.openclaw-rp-plugin.config.agentHarness.runAttemptDiagnostics=true`. It registers `openclaw-rp-runattempt-diagnostic`, can filter by `runAttemptProvider` and `runAttemptModel`, claims matching harness selections, logs sanitized `runAttempt` parameter shape, and returns a controlled diagnostic response. Use only on the dedicated RP agent while testing because it replaces the normal model reply for matching calls.
